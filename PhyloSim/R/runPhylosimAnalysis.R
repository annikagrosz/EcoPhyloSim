...
#' @param parameterFile Path to a file containing the parameters in form of a data.frame
#' @param summaryFile Path to a file containing the (scaled) summary statistics
...
#' @param saABC Logical, determining whether saABC should be used to reduce the number of summaries.
...
# TODO: cleanup and make more generic. E.g. ^2, saABC, etc should be optional.
#       Further the user should be able to pass data.frames instead of filenames
rejectionSamplePhylosimAnalysis <- function (parameterFile, summaryFile, nDraws="auto", eQuantile=0.025, nCores="auto", saABC=FALSE, sumFun=median) {
  parameters <- PhyloSim:::loadLocal(parameterFile)
  summaries <- PhyloSim:::loadLocal(summaryFile)
  
  if (saABC) {
    summaryNames <- c(names(summaries), paste(names(summaries), "^2", sep = ""))
    summaries <- cbind(summaries, summaries^2)
    names(summaries) <- summaryNames
    
    saABCWeights <- abctools::saABC(parameters, summaries, plot = FALSE)$B
    
    if (any(is.na(saABCWeights))) {
      x <- apply(X = saABCWeights, MARGIN = 2,
                 FUN = function(x) return(!any(is.na(x))))
      warning(paste("Linear regression problems: ill-conditioning? Ignored summaries leading to NAs! (", paste(names(x)[!x], collapse = ", "), ")"))
      saABCWeights <- abctools::saABC(parameters, summaries[, x], plot = FALSE)$B
    }
    
    summaries <- as.matrix(summaries[, x]) %*% t(saABCWeights)
  }
  
  parameters <- as.matrix(parameters)
  summaries <- as.matrix(summaries)
  
  rsResult <- rejectionSample(parameters, summaries, nDraws, eQuantile, nCores, sumFun)
}
