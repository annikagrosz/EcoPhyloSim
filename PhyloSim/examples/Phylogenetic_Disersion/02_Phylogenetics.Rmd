...
for(i in 1:28){
    for(k in 1:10){
      SpeciesRichness100[i,k] <- specRich(simulationOut100[[i]], which.simulation = k)
      SpeciesRichness256[i,k] <- specRich(simulationOut256[[i]], which.simulation = k)
    }
}
...
for(i in 1:28){
  for(k in 1:10){
    colless100[i,k] <- collessImbalance(simulationOut100[[i]], which.simulation=k)
    colless256[i,k] <- collessImbalance(simulationOut256[[i]], which.simulation=k)   
  }
}
