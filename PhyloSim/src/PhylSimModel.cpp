...
PhylSimModel::PhylSimModel(int X, int Y, int dispersal, int simulationEnd, double specRate, bool dens, 
   ...
   if (dispersal == 1) {    
      m_Global = new GlobalEnvironment(X,Y, dispersal, neutral, dens, env, mort, repro, simulationEnd, specRate, dispersalCutoff, densityCutoff, mortalityStrength, envStrength, compStrength, fission, redQueen, redQueenStrength, protracted);
      m_Local = NULL;
   } else if (dispersal == 2 || dispersal == 3) {
      m_Global = NULL;
      m_Local = new LocalEnvironment(X,Y, dispersal, neutral, dens, env, mort, repro, simulationEnd, specRate, dispersalCutoff, densityCutoff, mortalityStrength, envStrength, compStrength, fission, redQueen, redQueenStrength, protracted);
   }
