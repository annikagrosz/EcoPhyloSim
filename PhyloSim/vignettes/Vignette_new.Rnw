...
The phylosim package represents an expansion of previous models by combining both, the spatial and the phylogenetic aspect, into one model.\\
...
respective resource (approch see: \citep{ackermann2004}).\\ Further each individual
possesses a neutral trait. This trait evolves over time but not is influencing the individual's properties.\\
Apart from the individual's traits each species has trait values. These are not independent but defined as the mean of the respective traits of all individuals of the species. 
...
This kernel represents the distance to the cell to be populated and is defined as:
\begin{equation}
w_d = exp\frac{1}{-\Delta_s/2}
\end{equation}
$w_d$ represents the weight derived from the euclidean distance $\Delta_s$ between the
...
The trait is modelled as the mean of a Gaussian Distribution (see above). In each generation this value is slightly changed. The new trait is mosty influenced by the parent's value. Further, a weighted random value and the attraction of the specie's value have an influence on the new trait. 
...
The basic structure of how the model is applied is shown in figure \ref{fig: run}.
\begin{figure}[h]
	\centering
...
If the Phylsim object contains multiple results, they can be accessed by the which.simulation argument in most functions. By default always the last result is used.\\
...
As input serves a list of parameter settings as created by createCompletePar, whereas the output is an object of tyoe PhyloBatch. Basically this object is a list of Phylosim objects.\\
...
\begin{figure}[h]
	\centering
	\includegraphics[width=10cm]{plots(1)} % without file extension
...
\marginnote{The plotTraitDistribution plot consists of three different figures for the three traits in the model (environmental trait, competition trait, neutral trait). Figure one illustrates the trait's magnitude in dependency of the environment for different species. Figure two illustrates the spatial distribution of the given trait. Figure three shows a histogram of the given trait.\\
...
\newpage
\section{Phylogeny Trees}
\subsection{Reconstructed Phylogenies}
The species' traits can be used to construct a phylogeny for a given community.
In the package this is deone with the phlyoReconstruct function. \\
...
\subsection{Highlight Local Communities}
Further, the package comprises a plot function to highlight the phylogeny of a local community within a metacommunity. The local community can be given to the function directly or is derived from the simu object via localPlots.

<<>>=
highlightLocalPhylo(simu, size = 50, n = 1)
@
\marginnote{Both phylogeny trees show the same metacommunity. But their local communites differ in plotsize (5 and 50). Theiy are highlighted  in black. It is obvious that the local community with larger plotsize includes more species than the smaller local community}
<<>>=
...
\subsection{SAC-Curve}
\marginnote{A positively bent curve indicates clustering of a species community. An increase in plot size leads to an increase in specis richness. A negatively bent curve indicates a more neutral distribution of species within the community.}
\begin{center}
<< results='hide', out.width='6cm', out.height='6cm', fig.show='hold'>>=
sac(simu, which.simulation = NULL)
@
\end{center}
\subsection{RAC-Curve}
\marginnote{RACs display the amount of equally abundand species that the community can support. A linear curve indicates a less stable or neutral community supporting only a few highly abundand species, whereas an S-shaped curve indicates a more stable community. In the latter case several species of the same abundance can be supported.}
...
\begin{figure}[h]
	\centering
	\includegraphics[width=10cm]{nullmodel} % without file extension
	\caption{Overview about the basic nullmodel functions in the PhyloSim package. Functions are represented by bubbles, objects by circles. Objects of the class PhylosimList can be indexed if a single Phylosim object is needed. The functions "calculatePhylogeneticDispersion" and "plotPhylogeneticDispersion" only make sence for objects of the class PhylosimList because they compare several model runs.}\label{fig: null}
\end{figure} 
\FloatBarrier


To tackle the question, whether there are patterns in the results of the 
simualtions, the package comprises a set of null models to test the observed results
against. For objects of type Phylosim these models are brought together in the nullModel function.\\
...
These subplots can now tested against randomly collected plots.\\
\section{nullModel function}
...
@
\section{Several model runs testet against null models}
...
@
The figure shows different combinations of dispersal limitation (y-axis), environmental trait(uppe x-axis) and density dependnce (lower x-axis). The respective colour shows the phylogenetic pattern (over- or underdispersed). 
