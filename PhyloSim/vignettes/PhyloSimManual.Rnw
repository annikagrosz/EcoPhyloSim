...
When it comes to analizing the phylogenies of species, it is often useful to look at their phylogeny trees to get an idea how the community evolved. Despite the plotSpatialPhylo function (which includes a phylogeny tree to get an overview rather than for analization) there are two more functions that create phylogeny trees. These are called phyloReconstruct and hihglightLocalPhylo. 
\subsection{Reconstructed Phylogenies}
The function phyloReconstruct uses the species' traits to construct a phylogeny for the given community.
These phylogenies can then be compared to the real phylogeny of the community:\\
...
Further, the package comprises a plot function to highlight the phylogeny of a local community within a metacommunity. This function is called hihglightLocalPhylo.
\newpage
...
The SAC shows the accumulated species richness as a function of the plot size while the RAC is an indicator for the stability of a community.
...
\newpage
\chapter{Appendix}
\section{How to compile the package in r}
...
3. In case devtools is not installed, execute:\\
