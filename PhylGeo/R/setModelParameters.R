...
setModelParametersXML <- function(scenarios, x, y, runs, dispersal, specRate, density, environment, mortalityFitness, reproductiveFitness, neutral, location, dispersalCut, densityCut, seed)
{
  parameters <- data.frame(scenarios = scenarios, x = x, y = y, runs = runs, dispersal = dispersal, specRate = specRate, density = as.integer(density), environment = as.integer(environment), mortalityFitness = as.integer(mortalityFitness), reproductiveFitness = as.integer(reproductiveFitness), neutral = as.integer(neutral), dispersalCut = dispersalCut, densityCut = densityCut, seed = seed)
  file <- location
