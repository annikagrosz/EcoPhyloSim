...
#' @param which.simulation Integer, determines which simulation should be plotted. By default the last one is used (see Details).
...
#' @details In every subplot the x-axis represents the size of the subplots used in the null models. Hence, it is recommended to use a vector of plot sizes in your \code{\link{calculatePhylogeneticDispersion}} setup. \cr\cr The which.simulation argument here is used on the input list of p-values. Hence, which.simulation = 1 plots the first list slice of the input here. This is not necessarily the first time step of of your parameter settings in \code{\link{runSimulationBatch}}. \cr\cr For an further explanation please see the Examples section. 
#' @examples 
