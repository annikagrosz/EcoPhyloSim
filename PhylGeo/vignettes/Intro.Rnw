...
Here the results are shown for the same simulation 1000 runs.
...
Further it is possible to calculate the species richness of the results. The result is calculated for the simulation after 500 and 1000 time steps.

<<>>=
specRich(simu, which.simulation = 1)
specRich(simu, which.simulation = 2)
@
...
<<>>=
locals<-localPlots(size=20, 4, matrix=simu, community=T)
@
