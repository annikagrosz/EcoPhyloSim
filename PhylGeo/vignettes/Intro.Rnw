...
\section{Background}
TODO References
The field of community phylogenetics is based on the idea that species properties are often
phylogenetically conserved. This means, that more recently diverged species have more similar traits and therefore more similar properties.\\
Based on this idea one can infer dominant processes of community assembly by comparing the properties of the regional to the local phylogeny.
These processes can be distinguished in two main classes of processes, environmental filtering and local interactions.
Environmental filtering selects at each location species with a favorable trait for the specific environment. This would lead to phytogenetically similar species co-occuring together.
On the other hand local interactions or competition should be more fierce for species with similar traits as this would lead to limiting sinilarity.\\
The comparision of the phylogenetic signal of an observed assemblage with the signal from an randomly generated assemblage (created from a null-model) can give some indication about the dominate assembly process. 
An overdispersed phylogeny can be interpreted as an assemblage dominated by environmental filtering, whereas a clustered phylogeny can be interpreted as an assemblage dominated by local competition (Weiher & Keddy, 1995; Webb, 2000).\\
Whereas previous approaches to this topic dealt with phylogenetic or spatial patterns (and produced numeral explanations how species assemblages evolved) the present approach gives the opportunity to combine these informations.
Among a neutral model the phylosim package can simulate dispersal limitation, environmental preferences, and local competition. This tackles the main criticism of previous approaches which focused on one mechanism (e.g. environmental filtering vs. competitive exclusion) and ignored the possibility that this mechanism might not be solely if at all responsible for the emerging community pattern.
...
Apart from the individual's traits each species has trait values. In the model they are not independent but defined as the mean of the respective traits of all individuals of the species. 
...
The model output consists of five matrices representing the community landscape. 
These are the distribution of species, the environment, the environmental as well as the competition traits of the species and TODO NeutralMat?.
Further a table is produced including all information about the species and the phylogeny in the newick format. To faciliate the analysis the phylogeny is pruned by removing all extinct leaves without any branching events. 
TODO check this section again. Especially the last paragraph!
...
In the following example an area of 50*50 cells provides the basis for one simulations 1000 generations. The results are stored after 500 and 1000runs. \\
<<>>=
...
The output is saved as a list. The first two list slices contain the results of the two simulation after 500 and 1000 runs, whereas the last list slice contains the parameter settings.
...
To analyze the results the package contains functions to calculate and visualize the rank-abundace curve (RAC) and species-area curve (SAC) for the output. Here the results are shown for the same simulation after 500 and 1000 runs.
