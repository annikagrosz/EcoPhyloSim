...
<<>>=
par <- createCompletePar(x = 50, y = 50, 
                         dispersal = "global" , runs = c(500,1000), density = 1,
                         environment = 0.5, specRate = 1, type="base")
@
...
Outpuit consists of two results, the simulation after 500 and 1000 generations.
...
str(Output1)
@
These are the species matrix, the trait matrix, the environmental matrix, the competition matrix and the neutral matrix as well as the phlogeny.\\
The parameter settings are only saved once. Additionaly, the run time is save with these settings.
<<>>=
str(simu$Model)
@
...
If the Phylsim object contains multiple results, they can in all functions be accessed by the which.simulation argument. By default always the end result is used.\\
...
par1 <- createCompletePar(x = 50, y = 50, dispersal = "global" ,
                          runs = 1000, density = 1, environment = 0.5, 
                          specRate = 1, type="base")

par2 <- createCompletePar(x = 50, y = 50, dispersal = "global",
                          runs = 1000, density = 1, environment = 2,
                          specRate = 1, type="base")

parmBatch <- list(par1, par2)

simuBatch <- runSimulationBatch(parmBatch, parallel = TRUE)
@
...
Here the results are shown for the same simulation after 1000 runs.
...
To tackle the question, wheather there are pattern in the results of the 
...
You can choose, wheather the null model should be based on the abundance distribution of the sample plots or not.\\
If the model is not based on the abundance distribution, there are two possible measures of the phylogeny that are used to compare the observed results to the null model. These are the Mean Pairwise Distance (mpd) or Fait's Phylogenetic Diversity (pd)\marginnote{Both have their origin in the picante package}.

<<>>=
pValues <- nullModel(simu = simu, abundance = FALSE, localPlotSize = 100, numberOfPlots = 10, repetitions = 100, fun="mpd")
@

To calculate null models for the output of the runSimulationBatch function the calculatePhylogeneticDispersion function is used. It can be seen as an extension to the nullModel function, that deals with lists of Phylosim objects and can also calculate two external null models.
